...
class ForecastEngine:
    ...
    def train(self, train_df: pd.DataFrame, model_name: str = "random_forest", random_state: int = 42) -> TrainResult:
        ...
        self.model_registry.register_metrics(model_name, metrics)
        return TrainResult(artifact=artifact, metrics=metrics)

    def predict(self, df: pd.DataFrame, model_name: str = "random_forest", confidence_level: float = 0.95) -> pd.DataFrame:
        ...
        residual_std = float((transformed[self.target_col] - transformed["prediction"]).std()) if len(transformed) > 1 else 1.0
        z = 1.96 if confidence_level >= 0.95 else 1.64
        transformed["lower"] = (transformed["prediction"] - z * residual_std).clip(lower=0)
        transformed["upper"] = transformed["prediction"] + z * residual_std
        return transformed[["date", "item_id", self.target_col, "prediction", "lower", "upper"]].copy()
